...
mydb=mysql.connector.connect(host='localhost',user='root',password='mysql1234',db='prm')
...
def update(uid):
    if session.get('user'):
        ...
        if request.method=='POST':
            ...
            content=request.form['content']
            cursor=mydb.cursor(buffered=True)
            cursor.execute('update notes set title=%s ,content=%s  where bin_to_uuid(nid)=%s',[title,uid,content])
            mydb.commit()
            cursor.close()
...
def filesview():
    if session.get('user'):
        username=session.get('user')
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select bin_to_uuid(fid) as uid,date from files where added_by=%s',[username])
        data=cursor.fetchall()
...
def filedelete(uid):
    if session.get('user'):
        cursor=mydb.cursor(buffered=True)
        cursor.execute('delete from notes where bin_to_uuid(fid)=%s',[uid])
        mydb.commit()
        cursor.close()
